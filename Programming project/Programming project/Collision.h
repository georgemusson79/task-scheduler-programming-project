...
namespace Collision {
	...
	//Checks if a position collides with a RenderableRect. Returns true if a position collides with a RenderableRect, otherwise returns false
	bool collidesWith(SDL_Rect rect,Vector2 pos);
	
	//Checks if 2 RenderableRects collide with each other, returns true if a collision is detected, otherwise returns false
